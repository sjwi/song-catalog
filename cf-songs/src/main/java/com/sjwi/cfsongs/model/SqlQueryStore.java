...
public class SqlQueryStore {
	...
	public SqlQueryStore(String dbClassName) throws FileNotFoundException {
			sqlQueries = (Map<String,String>) new Yaml().load(new FileReader(ResourceUtils.getFile(
					 dbClassName.contains("mysql") ? SQL_DIR_CLASSPATH + MYSQL_YML_FILENAME : SQL_DIR_CLASSPATH + MSSQL_YML_FILENAME)));
	}
