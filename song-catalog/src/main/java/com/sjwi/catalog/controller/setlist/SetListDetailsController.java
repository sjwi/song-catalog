...
public class SetListDetailsController {
	...
	public static final int SET_LISTS_PER_PAGE = 40;
	...
	public ModelAndView setListPage(
			...
			@RequestParam(name = "additionalSets",required=false) boolean additionalSets,
		...
		try {
			mv.addObject("sets", additionalSets? setListService.getSetListPage(SET_LISTS_PER_PAGE): setListService.getSetLists(SET_LISTS_PER_PAGE));
			mv.addObject("orgs",organizationService.getOrganizations());
			return mv;
		} catch (Exception e) {
			return controllerHelper.errorHandler(e);
