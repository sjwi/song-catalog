...
public class TransposableString {
  ...
  private static final List<String> CHORD_CHART_KEYWORDS =
      new ArrayList<String>(
          Arrays.asList("min", "maj", "dim", "minor", "major", "sus", "dom", "aug"));
  private static final List<String> STRIPPABLE_KEYWORDS =
      new ArrayList<String>(
          Arrays.asList(
              "verse",
              "chorus",
  ...
  private String transposeStringLines(KeySet transpositionKey) {
    return StringUtils.stripEnd(
        transposableStringLines.stream()
            .map(
                line ->
                    isLineOnlyChords(line) && !storedKey.getKeyId().equals(LYRICS_ONLY_KEY_CODE)
                        ? transposeChordLine(line, storedKey, transpositionKey)
                        : line)
  ...
  public static boolean isLineOnlyChords(String line) {
    ...
    if (line.contains("j")) {
      if (!doesLineContainChordKeyword(line, "j")) {
        return false;
      }
    }
    if (line.contains("k")) {
    ...
    if (line.contains("n")) {
      if (!doesLineContainChordKeyword(line, "n")) {
        return false;
      }
    }
    if (line.contains("o")) {
    ...
    if (line.contains("s")) {
      if (!doesLineContainChordKeyword(line, "s")) {
        return false;
      }
    }
    if (line.contains("t")) {
    ...
    if (line.contains("u")) {
      if (!doesLineContainChordKeyword(line, "u")) {
        return false;
      }
    }
    if (line.contains("w")) {
  ...
  private static boolean doesLineContainChordKeyword(String line, String character) {
    ...
    return CHORD_CHART_KEYWORDS.stream()
        .anyMatch(
            c ->
                StringUtils.containsIgnoreCase(c, character)
                    && StringUtils.containsIgnoreCase(line, c));
  }
  ...
  private String transposeChordLine(String line, KeySet defKey, KeySet transKey) {
    ...
    line =
        line.replace(defKey.getiSharp() + " ", transKey.getiSharp() + " ")
  ...
  private String standardizeUnicodeCharacters(String transposableString) {
    return StringUtils.stripEnd(
        transposableString.replaceAll("\u266D", "b").replaceAll("\u266F", "#"), null);
  }
  ...
  private String stripKeywordsFromLine(String line) {
    ...
    String keywordSubString =
        l.replaceAll("[^a-zA-Z ]", "").replaceAll("(?i)repeat", "").replaceAll(" ", "").trim();
  ...
  private List<String> trimExcessiveEmptyLines(List<String> formattedLines) {
    formattedLines =
        new ArrayList<String>(
            Arrays.asList(
                String.join(APPEND_LINE_DELIMITER, formattedLines)
                    .replaceAll("[\n]{3,}", "\n\n")
  ...
  private KeySet getKeyFromMaster(String keyId) {
    return KeySetConfiguration.getKeySetMaster().stream()
        .filter(k -> keyId.equalsIgnoreCase(k.getKeyId()))
        .findAny()
        .orElse(null);
  }
}
