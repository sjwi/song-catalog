package com.sjwi.cfsongs.file.ppt;
...
public class PowerPointDeck {
	...
	private List<List<String>> getSlidesFromChunk(String chunk) {
		...
		return combineSlidesIntoFinalSubDeck(
				reduceChunksToLinesLessThanLineSizeMax(chunkSplitIntoLines, 
						getDefaultLinesPerSlide(chunkSplitIntoLines.size())));
	}
	...
	private int splitStringAtEndOfWord(String line) {
		int index = 0;
		while (line.charAt(defaultLineSize + index) != ' ' && line.charAt(defaultLineSize + index) != NEW_LINE_DELIMITER.charAt(0)) {
			index = index + 1;
		}
		if (index > 10) {
			index = 0;
	...
	private List<List<String>> partitionListIntoSubLists(List<String> list, int partitionSize)
	{
		...
		for (int i = 0; i < numberOfLists; i++)
		{
			...
			int toIndex = (i*partitionSize + partitionSize < size) ? (i*partitionSize + partitionSize) : size;
	...
	private int getDefaultLinesPerSlide(int size) {
		while (size >= MAX_NUMBER_OF_LINES_PER_PAGE)
			size = size / 2;
