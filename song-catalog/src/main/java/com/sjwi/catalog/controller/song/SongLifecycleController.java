...
public class SongLifecycleController {
  ...
  @ResponseBody
  public ResponseMessage createSong(
    ...
    try {
      ...
      int id =
          songService.addSong(
              controllerHelper.titleCase(songTitle),
              songBody,
              chordedIn,
              userService.loadCfUserByUsername(principal.getName()),
              category);
      if (songAudio != null) {
  ...
  public ModelAndView getSongDetailsForEdit(
      @PathVariable int id,
      Authentication auth,
      HttpServletRequest request,
      HttpServletResponse response) {
    ...
    try {
      ...
      Song revisedSong =
          new MasterSong(
              ...
              originalSong.getId(),
              songTitle,
              new TransposableString(songBody, defaultKey),
              updatedKey,
              originalSong.getArtist(),
              originalSong.getNotes(),
              originalSong.getCreatedBy(),
              userService.loadCfUserByUsername(principal.getName()),
              new Date(),
              originalSong.getRelated(),
              originalSong.isPriv(),
              category,
              originalSong.getRecording());
      if (newVersion != null)
        id =
      ...
      if (songAudio != null) recordingService.addOrUpdateRecording(id, songAudio);
  ...
  @RequestMapping(
      value = {"song/delete/recording/{id}"},
      method = RequestMethod.DELETE)
  @ResponseStatus(value = HttpStatus.OK)
  public void deleteSongRecording(
      HttpServletRequest request,
      HttpServletResponse response,
      Authentication auth,
      @PathVariable int id) {
  ...
  @RequestMapping(
      value = {"song/add/recording/{id}"},
      method = RequestMethod.POST)
  @ResponseStatus(value = HttpStatus.OK)
  public void addSongRecording(
      HttpServletRequest request,
      HttpServletResponse response,
      Authentication auth,
