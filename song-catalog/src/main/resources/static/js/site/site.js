...
const FLOOR = 50;
const CEIL = 150;
const CLASS_PREFIX = 'song-fs-'
const SELECTOR_PREFIX = '.' + CLASS_PREFIX
$(document).ready(function(e){
	...
	$(document).on('click', '.zoom-min-sel', function(e){
		var curScale = getCookie('fontscale')
		if (!curScale) {
			curScale = '100'
		}
		nextScale = parseInt(curScale) - 10
		if (nextScale <= FLOOR)
			nextScale = FLOOR

		$('.managed-font').removeClass(CLASS_PREFIX + curScale);
		$('.managed-font').addClass(CLASS_PREFIX + nextScale.toString());

		$('#zoomPerc').html(nextScale);

		setFontScale(nextScale.toString())
		// $('.wrapper').css('height','auto');
		// $('.wrapper').css('height',$(document).height() - 30);
	});
	$(document).on('click', '.zoom-plus-sel', function(e){
		var curScale = getCookie('fontscale')
		if (!curScale) {
			curScale = '100'
		}
