...
public class VersionSongController {
    ...
    try {
      int versionId =
          versionService.createNewVersion(
              songId,
              principal.getName(),
              new TransposableString(versionBody, defaultKey)
      ...
      if (setSongId != 0) setListService.changeVersion(setSongId, versionId);
      if (songAudio != null) recordingService.addOrUpdateRecording(versionId, songAudio);
      logger.mailRevisionDeltas(
          songService.getSongById(songId),
          songService.getSongById(versionId),
          MailConstants.VERSION_CREATED_ACTION);
      return versionId;
    } catch (Exception e) {
  ...
  @RequestMapping(
      value = {"/song/get-all-related/{id}"},
      method = RequestMethod.GET)
  public ModelAndView getAllRelatedSongs(
      @PathVariable int id,
      Authentication auth,
      HttpServletRequest request,
      HttpServletResponse response) {
  ...
  @RequestMapping(
      value = {"/song/version/swap-master/{id}"},
      method = RequestMethod.POST)
  @ResponseStatus(value = HttpStatus.OK)
  public void swapMaster(
      @PathVariable int id,
      @RequestParam(value = "newId", required = true) int newId,
      Authentication auth,
      HttpServletRequest request,
      HttpServletResponse response) {
    try {
