...
public class SqlTokenDao implements TokenDao {
  ...
  public void storeEnrollmentToken(EnrollmentToken token) {
    jdbcTemplate.update(
        queryStore.get("storeToken"),
        new Object[] {token.getTokenString(), token.getUser(), token.getRole()});
  }

  @Override
  public EnrollmentToken getEnrollmentToken(String user) {
    return jdbcTemplate.query(
        queryStore.get("getTokenByUser"),
        new Object[] {user},
        r -> {
          if (r.next()) {
  ...
  public SecurityToken getStoredCookieToken(String token) {
    return jdbcTemplate.query(
        queryStore.get("getCookieToken"),
        new Object[] {token},
        r -> {
          if (r.next()) {
            return new StoredCookieToken(
                r.getInt("ID"),
                r.getString("username"),
                r.getString("LOGIN_COOKIE"),
  ...
  public void storeCookieToken(SecurityToken token) {
    jdbcTemplate.update(
        queryStore.get("storeCookieToken"), new Object[] {token.getUser(), token.getTokenString()});
  }
  ...
  public void storePasswordResetToken(SecurityToken token) {
    ...
    jdbcTemplate.update(
        queryStore.get("storePasswordResetToken"),
        new Object[] {token.getUser(), token.getTokenString()});
  }

  @Override
  public PasswordResetToken getPasswordResetToken(String username) {
    return jdbcTemplate.query(
        queryStore.get("getPasswordResetToken"),
        new Object[] {username},
        r -> {
          if (r.next()) {
