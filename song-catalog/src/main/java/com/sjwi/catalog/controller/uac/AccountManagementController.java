...
public class AccountManagementController {
  ...
  @ResponseBody
  public ResponseMessage sendInvite(
      @RequestParam(name = "invitee", required = true) String invitee,
      @RequestParam(name = "role", required = true) String role,
      Authentication auth,
      HttpServletRequest request) {
    try {
      if (userService.getAllActiveUsers().stream()
          .anyMatch(u -> u.getEmail().equalsIgnoreCase(invitee)))
  ...
  @ResponseBody
  public ResponseMessage sendResetPasswordEmail(
  ...
  @ResponseBody
  public ResponseMessage requestAccount(
      @RequestParam(name = "email", required = true) String email, HttpServletRequest request) {
    try {
      ...
      if (userService.loadUserByUsername(email) != null)
        throw new BadAttributeValueExpException(
            "An existing account is already associated with " + email);
  ...
  public ModelAndView promptForPasswordReset(
    ...
    try {
      ...
      SecurityToken securityToken =
          tokenService.getPasswordResetToken(cfUser.getUsername()).setSessionToken(token);
