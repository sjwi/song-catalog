...
public class SongDetailsController {
  ...
  public ModelAndView songCatalog(
    ...
    try {
      ...
      if (songs.size() == 0) return new ModelAndView("redirect:/?searchTerm=" + searchTerm);
      else if (songs.size() == 1) return new ModelAndView("redirect:/song/" + songs.get(0).getId());
      else {
  ...
  @RequestMapping(
      value = {"/song/details/{id}"},
      method = RequestMethod.GET)
  public ModelAndView songDetails(
      @PathVariable int id,
      ...
      if (key != null) {
        focusedSong =
            focusedSong.stream()
                .map(
                    s -> {
                      if (s.getId() == id) s = s.transpose(key);
                      return s;
                    })
                .collect(Collectors.toList());
      }
