...
public class PasswordResetToken extends StoredCookieToken {
  ...
  private PasswordResetToken(
      int id, String user, String token, Date createdOn, String sessionToken) {
    super(id, user, token, createdOn);
  ...
  public boolean isTokenValid() {
    ...
    return sessionToken != null && token != null
        ? createdOn.compareTo(cal.getTime()) > 0 && sessionToken.equals(token)
        : false;
  }

  @Override
  public String getTokenLink() {
    return ServletConstants.FULL_URL
        + "/"
        + VALIDATION_ENDPOINT
        + "?token="
        + token
